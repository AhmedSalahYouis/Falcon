...
class LaunchListUseCaseTest {
    ...
    fun `invoke returns expected flow`() = runBlocking {
        val expectedFlow: Flow<PagingData<LaunchSummary>> = flowOf(PagingData.from(listOf(LaunchSummary("1", "Falcon 9", Mission("test mission", "")))))
    ...
    fun `invoke with different params returns correct flow`() = runBlocking {
        val expectedFlow: Flow<PagingData<LaunchSummary>> = flowOf(PagingData.from(listOf(LaunchSummary("2", "Falcon 1", Mission("test mission", "")))))
    ...
    @Test
    fun `repository getLaunchesList is called with correct param`(): Unit = runBlocking {
        val expectedFlow: Flow<PagingData<LaunchSummary>> = flowOf(PagingData.empty())
        `when`(repository.getLaunchesList(3)).thenReturn(expectedFlow)
        useCase.invoke(GetLaunchListUseCase.Params(3))
        verify(repository).getLaunchesList(3)
    }
}